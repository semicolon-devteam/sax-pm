...
> **🔔 시스템 메시지**: 이 Skill이 호출되면 `[SAX] Skill: health-check 호출 - PM 환경 검증` 시스템 메시지를 첫 줄에 출력하세요.
...
신규/기존 PM의 개발 환경을 자동으로 검증하여 SAX 사용 준비 상태를 확인합니다.
...
# SAX 패키지 설치 상태 확인
ls -la .claude/sax-pm/ 2>/dev/null && echo "✅ sax-pm 설치됨"
...
| 항목 | 검증 방법 |
|------|----------|
| 패키지 디렉토리 | `.claude/sax-core/`, `.claude/sax-pm/` 존재 확인 |
| CLAUDE.md 심링크 | `.claude/CLAUDE.md` → `sax-pm/CLAUDE.md` |
| agents 심링크 | `.claude/agents` → `sax-pm/agents` |
| skills 심링크 | `.claude/skills` → `sax-pm/skills` |
| commands 심링크 | `.claude/commands/SAX` → `../sax-pm/commands` |
...
[SAX] Reference: PM 환경 검증 (도구/인증/GitHub Projects) 완료
```
